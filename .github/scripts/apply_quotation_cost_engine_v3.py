...
def inject_css_links(html: str) -> str:
    ...
    if LIVE_FX_LINK_MARKER not in html:
        links.append('<link rel="stylesheet" href="assets/quotation-live-fx-v1.css" data-conutway-live-fx="v1">')
    if not links:
        return html
...
def main() -> int:
    ...
    live_fx_source = repo_root / ".github" / "pages" / "quotation-live-fx-v1.js"
    ...
    live_fx_css_source = repo_root / ".github" / "pages" / "quotation-live-fx-v1.css"
    catalog_source = repo_root / ".github" / "pages" / "cost-catalog-v3.json"

    required = [
        app_path, index_path, fallback_path, engine_source, auto_source, ncm_sync_source,
        ncm_fix_source, live_fx_source, css_source, ncm_css_source, live_fx_css_source,
        catalog_source,
    ]
    ...
    live_fx = live_fx_source.read_text(encoding="utf-8")
    css = css_source.read_text(encoding="utf-8")
    ncm_css = ncm_css_source.read_text(encoding="utf-8")
    live_fx_css = live_fx_css_source.read_text(encoding="utf-8")
    if (
        ENGINE_MARKER not in engine
        or AUTO_MARKER not in auto
        or NCM_SYNC_MARKER not in ncm_sync
        or NCM_FIX_MARKER not in ncm_fix
        or LIVE_FX_MARKER not in live_fx
        or CSS_MARKER not in css
        or NCM_CSS_MARKER not in ncm_css
        or LIVE_FX_CSS_MARKER not in live_fx_css
    ):
        raise RuntimeError("Marcadores do motor V3/NCM/câmbio ao vivo não encontrados nos arquivos-fonte.")
    ...
    if ENGINE_MARKER not in app:
        ...
        if LIVE_FX_MARKER not in app:
            additions.append(live_fx)
        if additions:
            if anchor not in app:
    ...
    shutil.copy2(live_fx_css_source, assets / "quotation-live-fx-v1.css")
    ...
    checks = {
        ...
        "live_fx": LIVE_FX_MARKER in final_app,
        ...
        "live_fx_css": LIVE_FX_CSS_MARKER in (assets / "quotation-live-fx-v1.css").read_text(encoding="utf-8"),
        "css_link": LINK_MARKER in final_index,
        "ncm_css_link": NCM_LINK_MARKER in final_index,
        "live_fx_css_link": LIVE_FX_LINK_MARKER in final_index,
        ...
        "live_fx_button": "data-live-fx-refresh" in final_app,
        "live_fx_bcb": "CotacaoDolarDia" in final_app,
        "product_tax_fields": 'data-product-field=\\"iiRate\\"' in final_app or 'data-product-field="iiRate"' in final_app,
    }
    failed = [name for name, ok in checks.items() if not ok]
    if failed:
        raise RuntimeError("Falha na integração do motor de custos V3/NCM/câmbio: " + ", ".join(failed))
    ...
    print(
        ...
        f"live_fx_css_bytes={(assets / 'quotation-live-fx-v1.css').stat().st_size}",
        "ncm_tax_sync=v1",
        "live_fx=v1",
    )
    return 0
