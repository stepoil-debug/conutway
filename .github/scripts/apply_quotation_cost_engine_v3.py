...
def main() -> int:
    ...
    required = [app_path, index_path, fallback_path, engine_source, auto_source, css_source, catalog_source]
    ...
    if ENGINE_MARKER not in engine or AUTO_MARKER not in auto or CSS_MARKER not in css:
        raise RuntimeError("Marcadores do motor V3 não encontrados nos arquivos-fonte.")
    ...
    elif AUTO_MARKER not in app:
        if anchor not in app:
            raise RuntimeError("Ponto de injeção initializeApplication não encontrado para auto-default.")
        app = app.replace(anchor, f"\n\n{auto}\n\n" + anchor, 1)
        app_path.write_text(app, encoding="utf-8")
    ...
    if failed:
        raise RuntimeError("Falha na integração do motor de custos V3: " + ", ".join(failed))
